    ...
    extends ImageRenderer(imgSize)
{
    ...
    private val ESCAPE_TIME_MAX = 300
    ...
    override def getColorFor(imgCoords: Point2i): ColorRGB = {
        ...
        val colorIdx =
            if (escapeTime < ESCAPE_TIME_MAX) (escapeTime.toFloat / ESCAPE_TIME_MAX * palette.maxIndex).toInt
            else 0
    ...
    private def getEscapeTime(c:Complex):Int = {
        ...
        while (i < ESCAPE_TIME_MAX && res.absSquared <= ESCAPE_THRESHOLD_SQR) {
            res = mapping(res, c)
            i = i + 1
        }
        i
